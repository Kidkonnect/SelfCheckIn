...
void Adafruit_PN532::begin() {
  ...
  //Keyboard.println("inside begin function");
  delay(1000);
  
  // not exactly sure why but we have to send a dummy command to get synced up
  pn532_packetbuffer[0] = PN532_COMMAND_GETFIRMWAREVERSION;
  sendCommandCheckAck(pn532_packetbuffer, 1);
  //Keyboard.println(PN532_COMMAND_GETFIRMWAREVERSION);
  // ignore response!
}
...
boolean Adafruit_PN532::writeGPIO(uint8_t pinstate) {
  ...
  #ifdef PN532DEBUG
    Keyboard.print("Writing P3 GPIO: "); Keyboard.println(pn532_packetbuffer[1], HEX);
  #endif
...
uint8_t Adafruit_PN532::readGPIO(void) {
  ...
  /* READGPIO response should be in the following format:
    ...
    Keyboard.println("");
    Keyboard.print("P3 GPIO: 0x"); Keyboard.println(pn532_packetbuffer[6], HEX);
    Keyboard.print("P7 GPIO: 0x"); Keyboard.println(pn532_packetbuffer[7], HEX);
    Keyboard.print("IO GPIO: 0x"); Keyboard.println(pn532_packetbuffer[8], HEX);
    // Note: You can use the IO GPIO value to detect the Keyboard bus being used
    switch(pn532_packetbuffer[8])
    {
      case 0x00:    // Using UART
        Keyboard.println("Using UART (IO = 0x00)");
        break;
      case 0x01:    // Using I2C 
        Keyboard.println("Using I2C (IO = 0x01)");
        break;
      case 0x02:    // Using SPI
        Keyboard.println("Using SPI (IO = 0x02)");
        break;
    }
  #endif
...
#ifdef MIFAREDEBUG
  Keyboard.print("Setting MxRtyPassiveActivation to "); Keyboard.print(maxRetries, DEC); Keyboard.println(" ");
#endif
...
#ifdef MIFAREDEBUG
    Keyboard.print("Found "); Keyboard.print(pn532_packetbuffer[7], DEC); Keyboard.println(" tags");
#endif
...
#ifdef MIFAREDEBUG
    Keyboard.print("ATQA: 0x");  Keyboard.println(sens_res, HEX); 
    Keyboard.print("SAK: 0x");  Keyboard.println(pn532_packetbuffer[11], HEX); 
#endif
...
#ifdef MIFAREDEBUG
      Keyboard.print(" 0x");Keyboard.print(uid[i], HEX); 
#endif
  }
#ifdef MIFAREDEBUG
    Keyboard.println();
...
uint8_t Adafruit_PN532::mifareclassic_AuthenticateBlock (uint8_t * uid, uint8_t uidLen, uint32_t blockNumber, uint8_t keyNumber, uint8_t * keyData)
{
  ...
  #ifdef MIFAREDEBUG
  Keyboard.print("Trying to authenticate card ");
  Adafruit_PN532::PrintHex(_uid, _uidLen);
  Keyboard.print("Using authentication KEY ");Keyboard.print(keyNumber ? 'B' : 'A');Keyboard.print(": ");
  ...
  {
    #ifdef PN532DEBUG
    Keyboard.print("Authentification failed: ");
...
{
  #ifdef MIFAREDEBUG
  Keyboard.print("Trying to read 16 bytes from block ");Keyboard.println(blockNumber);
  #endif
  ...
  {
    #ifdef MIFAREDEBUG
    Keyboard.println("Failed to receive ACK for read command");
    #endif
    return 0;
  }
  ...
  {
    //#ifdef MIFAREDEBUG
    Keyboard.println("Unexpected response");
  ...
  #ifdef MIFAREDEBUG
    Keyboard.print("Block ");
    Keyboard.println(blockNumber);
...
{
  #ifdef MIFAREDEBUG
  Keyboard.print("Trying to write 16 bytes to block ");Keyboard.println(blockNumber);
  #endif
  ...
  {
    #ifdef MIFAREDEBUG
    Keyboard.println("Failed to receive ACK for write command");
    #endif
    return 0;
  }  
  delay(10);
...
uint8_t Adafruit_PN532::mifareultralight_ReadPage (uint8_t page, uint8_t * buffer)
{
  ...
  {
    #ifdef MIFAREDEBUG
    Keyboard.println("Page value out of range");
    #endif
    return 0;
  }

  #ifdef MIFAREDEBUG
    Keyboard.print("Reading page ");Keyboard.println(page);
  #endif
  ...
  {
    #ifdef MIFAREDEBUG
      Keyboard.println("Unexpected response reading block: ");
      Adafruit_PN532::PrintHexChar(pn532_packetbuffer, 26);
    #endif
    return 0;
  }
  ...
  /* Display data for debug if requested */
  #ifdef MIFAREDEBUG
    Keyboard.print("Page ");Keyboard.print(page);Keyboard.println(":");
...
void Adafruit_PN532::spiwritecommand(uint8_t* cmd, uint8_t cmdlen) {
...
#ifdef PN532DEBUG
  Keyboard.print(" 0x"); Keyboard.print(PN532_PREAMBLE, HEX);
  Keyboard.print(" 0x"); Keyboard.print(PN532_PREAMBLE, HEX);
  Keyboard.print(" 0x"); Keyboard.print(PN532_STARTCODE2, HEX);
  Keyboard.print(" 0x"); Keyboard.print(cmdlen, HEX);
  Keyboard.print(" 0x"); Keyboard.print(~cmdlen + 1, HEX);
  Keyboard.print(" 0x"); Keyboard.print(PN532_HOSTTOPN532, HEX);
#endif
  ...
  for (uint8_t i=0; i<cmdlen-1; i++) {
   ...
   checksum += cmd[i];
#ifdef PN532DEBUG
   Keyboard.print(" 0x"); Keyboard.print(cmd[i], HEX);
#endif
  }
...
#ifdef PN532DEBUG
  Keyboard.print(" 0x"); Keyboard.print(~checksum, HEX);
  Keyboard.print(" 0x"); Keyboard.print(PN532_POSTAMBLE, HEX);
  Keyboard.println();
#endif
} 
/************** low level SPI */
